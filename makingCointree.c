...
void makingCointree(){
	...
	TFile *file2 = new TFile(Form("%s_pair.root",filename),"read");
...
//	TTree *tree2 = (TTree*) file2->Get("Na22_pair");
	...
	TFile *file3 = new TFile(Form("%s_Pair_Coin_TDC.root",filename),"recreate");
	...
	tree3->Branch("rPH_ch0",&rPH_ch0);
	tree3->Branch("rPH_ch1",&rPH_ch1);
	...
	for (ient=0; ient<Entries2; ient++){
		...
		rPH_ch0 = pulseheight(waveform);
		if(rPH_ch0<=100)	continue;
		...
		rPH_ch1 = pulseheight(waveform);
		if(rPH_ch1<=100)	continue;
