...
#include "./include/pulse_analysis_functions_520.c"
...
void zerosupdatareadingOnline_1Ghz_520(){
...
//	const double ch1Ecut_mean = 95000;
//	const double ch1Ecut_FWHM = 12000;
//	const double ch0Ecut_mean = 96500;
//	const double ch0Ecut_FWHM = 12000;
...
//	gStyle->SetOptStat(0);
	...
	c2->cd(1)->SetGridx();
	c2->cd(1)->SetGridy();
	TH2D *h2_QDCch1_QDCch0 = new TH2D("Co60 QDC(ch1) by QDC(ch0)", "Co60 QDC(ch1) by QDC(ch0);QDC,ch0 (qdc);QDC,ch1 (qdc)",2000,0,QDCrange,2000,0,QDCrange);
	...
	c2->cd(5)->SetLogz();
	TH2D *h2_TDCdif_QDCch0 = new TH2D(Form("Co60 TDCcf dif(ch0-ch1) by QDC(ch0), (ch1 %.0fqdc)",ch1Ecut_mean),Form("Co60 TDCcf dif by ch0 QDC (ch1 %.0fqdc);QDC,ch0(qdc);TDC dif, ch0-ch1(ns)",ch1Ecut_mean),2000,0,QDCrange,600,-15,15);
	...
	c2->cd(6)->SetLogz();
	TH2D *h2_TDCdif_QDCch0_nocut = new TH2D("Co60 TDCcf dif(ch0-ch1) by QDC(ch0),nocut ","Co60 TDCcf dif by ch0 QDC, nocut;QDC,ch0(qdc);TDC dif, ch0-ch1(ns)",2000,0,QDCrange, 600, -15, 15);
...
//	cout<<"Filesize = "<<Filesize<<endl;
	uint32_t numevent = Filesize/(4*(waveform_length+6));
	uint32_t remainder = Filesize%(4*(waveform_length+6));
...
//	for (pqrj_flag[0]=0; ient<numevent; ){
	for (pqrj_flag[0]=0; CoinEventNumber<9000000; ){
		...
		numevent = EndByte/(4*(waveform_length+6));
		fseek(datafile,CurrByte,SEEK_SET);
		if(EndByte-CurrByte>=(4*(waveform_length+6))){
			fread_flag=1;
		...
		else if(EndByte-CurrByte<(4*(waveform_length+6))){
			printf("(ONLINE)");
			fflush(stdout);
			fread_flag=0;
		}
