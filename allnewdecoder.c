...
//tree
	...
	TFile* treefile = new TFile(Form("../../DecodedDataFiles/%s_class.root",filename),"recreate");
...
//////////////////////
	...
	for (pqrj_flag[0]=0; ; ){
		...
		if(EndByte-CurrByte>=sample_byte){
			...
			pulseBuffer = new Pulse1024(header,waveformBuffer);	leakage++;
		...
		if(pulseBuffer->EventNumber > EventNumber && NHitCh!=0){
			event_flag=1;
