...
void KeyboardCommand(int pqrj_flag[4]){		
	if(!kbhit())	return;
	int key = getch();
	switch(key){
		case 'p' :						// 'p'ause
...
void PMT_primary_attHVtest(){
	//char filename[90]="PMT_set2_primary_Na22_run02_HV_1480_1030_1480_1100_att_12_00_17_00";
	char filename[90]="PMT_set2_primary_Cs137_run03_HV_1480_1030_1480_1100_att_12_00_17_00";
	//char filename[90]="PMT_set2_primary_Na22_HV_1470_1050_1480_1120_att_11_00_16_00_run02";
	//char filename[90]="PMT_set2_primary_Cs137_HV_1470_1050_1480_1120_att_11_00_16_00";
...
//	cout<<"Filesize = "<<Filesize<<endl;
	...
	printf("%s.dat \nFilesize: %lld(Bytes), numevent: %u, Error:%u\n", filename, Filesize, numevent, remainder);
...
//	for (pqrj_flag[0]=0; ient<numevent; ){
	...
	TF1 *Na511[NOpCh];
	double fitpar[5] = {300, 80000,1500,0,0};	//Na22
	//double fitpar[5] = {400,105000,1500,0,0};	//Cs137
	for (int ich=0; ich<NOpCh; ich++){
		printf("\nch%d\n",ich);
		c1->cd(ich+1+1*NOpCh);
		Na511[ich] = new TF1(Form("Na511_ch%d",ich),"gaus(0)+pol1(3)",  75000, 95000);  	//Na22
		//Na511[ich] = new TF1(Form("Na511_ch%d",ich),"gaus(0)+pol1(3)",  95000,140000);  	//Cs137
		Na511[ich]->SetParameters(fitpar);
		Na511[ich]->SetParLimits(1, 77000, 90000);	//Na22
		//Na511[ich]->SetParLimits(1, 95000,130000);	//Cs137
		Na511[ich]->SetParLimits(2,1000,3000);
		h1_QDC[ich]->Fit(Na511[ich],"M+","", 75000, 90000);	//Na22
		//h1_QDC[ich]->Fit(Na511[ich],"M+","", 98000,130000);	//Cs137
		h1_QDC[ich]->Fit(Na511[ich],"M+","",Na511[ich]->GetParameter(1)-2*Na511[ich]->GetParameter(2),Na511[ich]->GetParameter(1)+2*Na511[ich]->GetParameter(2));
	}
	printf("{");
	for (int ich=0; ich<NOpCh; ich++){
		printf("{%e,%e},",Na511[ich]->GetParameter(1),Na511[ich]->GetParameter(2));
	}
	printf("\b}\n");
	for (int ich=0; ich<NOpCh; ich++){
		printf("Eres ch%d = %.2f(%.0f)%%\n",ich, Na511[ich]->GetParameter(2)/Na511[ich]->GetParameter(1)*235,(Na511[ich]->GetParameter(2)/Na511[ich]->GetParameter(1))*sqrt(pow(Na511[ich]->GetParError(2)/Na511[ich]->GetParameter(2),2)+pow(Na511[ich]->GetParError(1)/Na511[ich]->GetParameter(1),2))*23500);
	}
