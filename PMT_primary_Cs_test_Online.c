...
void PMT_primary_Cs_test_Online(){
	char filename[90]="PMT_primary_test_setvoltage_set0";
...
//canvas1
	...
	for (int ich=0; ich<NOpCh; ich++){
		...
		c1->cd(ich+1)->SetGridx();	h1_rPH[ich]->Draw();
		c1->cd(ich+5)->SetGridx();	h1_QDC[ich]->Draw();
	}
...
//	for (pqrj_flag[0]=0; ient<numevent; ){
		...
		if(Elap_Time > 1000){
			...
			if(pqrj_flag[2] || pqrj_flag[3]){
				for (int jch=0; jch<NOpCh; jch++){
					c1->cd(2*jch+1);	gPad->Modified();	gPad->Update();
					c1->cd(2*jch+2);	gPad->Modified();	gPad->Update();
				}
			}
			if(pqrj_flag[3]){
				printf("\n");
	...
	for (int ich=0; ich<NOpCh; ich++){
		printf("\nch%d\n",ich);
		Cs662[ich] = new TF1(Form("Cs662_ch%d",ich),"gaus(0)+pol1(3)", 100000,120000);
		Cs662[ich]->SetParameters(fitpar);
		Cs662[ich]->SetParLimits(1,100000,130000);
		Cs662[ich]->SetParLimits(2,1000,3000);
		h1_QDC[ich]->Fit(Cs662[ich],"M+","",100000,120000);
		h1_QDC[ich]->Fit(Cs662[ich],"M+","",Cs662[ich]->GetParameter(1)-2*Cs662[ich]->GetParameter(2),Cs662[ich]->GetParameter(1)+2*Cs662[ich]->GetParameter(2));
	}
	for (int ich=0; ich<NOpCh; ich++){
		printf("Eres ch%d=%.2f\n", ich, 235*Cs662[ich]->GetParameter(2)/Cs662[ich]->GetParameter(1));
	}
