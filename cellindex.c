...
//	cout<<"Filesize = "<<Filesize<<endl;
	...
	c2->cd()->SetGridy();
	//c2->cd()->SetLogz();
	TH2D *h2_wft_ADC = new TH2D("wft_ADC","wft_adc;waveformtime(4sample);ADC(1V/4096)",1024,0,1024,200,-100, 100);
	//TH2D *h2_wft_ADC = new TH2D("wft_ADC","wft_adc;waveformtime(4sample);ADC(1V/4096)",1024,0,1024,200,3880,4090);
...
//		if(EventNumberch[0]==EventNumberch[1] && TrigTimeTagch[0]==TrigTimeTagch[1]){
			...
			//if(rPHch[0]<300){
				...
				pedline = new TLine(0,pulseavg,450,pulseavg);
				...
				TGraph *gr2 = new TGraph( 950,waveformtime,waveform2ch[0]);
				gr2->GetYaxis()->SetRangeUser(pulseavg-100,pulseavg+100);
				//gr2->GetYaxis()->SetRangeUser(3880,4096);
				gr2->SetLineColor(2);
				gr2->Draw();
				//gr2->Draw("SAME");
