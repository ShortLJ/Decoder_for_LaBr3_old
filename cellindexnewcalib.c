...
//void cellindexcalib(char filename[90], float cellcorravg[1024]);




void cellindexcalib(TString filename, Float_t cellcorravg[1024], Float_t celloffsetavg[1024], int pqrc_flag[3]){
	FILE *datafile = fopen(filename, "rb");	// moved from latter to here
	if(datafile==NULL){
		fputs("File error\n",stderr);
		exit(1);
	}
	//printf("%s.dat successfully opened. \n", filename);
	//printf(filename + " successfully opened. \n");
	cout << filename << ".dat successfully opened." << endl;
		...
		if(event_flag){
			...
			if(1){
				...
				for(int iwft=0; iwft< 950; iwft++){	
					...
					cellcorravg[(StartIndexCell+iwft)%1024]=((float)avgidx*cellcorravg[(StartIndexCell+iwft)%1024]+waveformch[0][iwft])/((float)avgidx+1);
					celloffsetavg[(StartIndexCell+iwft)%1024]=((float)avgidx*celloffsetavg[(StartIndexCell+iwft)%1024]+waveformch[0][iwft]-pulseavg)/((float)avgidx+1);
	...
	TLatex *tltx1 = new TLatex( 700, pulseavg+50, "#splitline{terminator 50#Omega}{#splitline{DC offset -16%}{new calib}}");
	tltx1->SetTextSize(0.03);
	tltx1->Draw("SAME");
	c2->cd();
	TLatex *tltx2 = new TLatex( 700, 50, "#splitline{terminator 50#Omega}{#splitline{DC offset -16%}{new calib}}");
	tltx2->SetTextSize(0.03);
	tltx2->Draw("SAME");

	c3->cd();
	TLatex *tltx3 = new TLatex( 7000,5000, "#splitline{terminator 50#Omega}{#splitline{DC offset -16%}{new calib}}");
...
void cellindexnewcalib(){
	const int numberofpoints = 13;
	int voltages[numberofpoints] = {0,25,50,100,150,250,350,450,550,650,750,850,950};
	Float_t cellavg[numberofpoints][1024];
	Float_t celloffsetavg[numberofpoints][1024];
	int pqrc_flag[3]={0,0,0};
	int asdf=0;

	for (int voltage : voltages){
		if(pqrc_flag[1]) break;
		//TString filename = Form("testforindex_obcalib_%d.dat",voltage);
		//TString filename = Form("testforindex_nocalib_%d.dat",voltage);
		TString filename = Form("testforindex_newcalib_%d.dat",voltage);
		cellindexcalib(filename, cellavg[asdf], celloffsetavg[asdf], pqrc_flag);
		asdf++;
	}
	int cellidxnumber;
	srand(time(NULL));
	printf("cellidxnumber ");
	TMultiGraph *mg4 = new TMultiGraph("cellavg","ADC avg of a cell;extDC(mV);ADC(1V/4096)");
	TMultiGraph *mg5 = new TMultiGraph("celloffsetavg","ADC offset avg of a cell;extDC(mV);ADC(1V/4096)");
	const int numgr=15;
	TGraph *gr4[numgr];
	TGraph *gr5[numgr];
	for (int i=0; i<numgr; i++){
		gr4[i] = new TGraph(numberofpoints);
		gr5[i] = new TGraph(numberofpoints);
		cellidxnumber = rand()%1023;
		printf("%d ",cellidxnumber);
		for (int j=0; j<numberofpoints; j++){
			//gr4[i]->SetPoint(j,4096.0*(float)voltages[j]/1000,cellavg[j][cellidxnumber[i]]);
			gr4[i]->SetPoint(j,voltages[j],cellavg[j][cellidxnumber]);
			gr5[i]->SetPoint(j,voltages[j],celloffsetavg[j][cellidxnumber]);
		}
		gr4[i]->SetMarkerStyle(5);
		gr4[i]->SetMarkerColor(i);
		gr4[i]->SetLineColor(i);
		mg4->Add(gr4[i]);
		gr5[i]->SetMarkerStyle(5);
		gr5[i]->SetMarkerColor(i);
		gr5[i]->SetLineColor(i);
		mg5->Add(gr5[i]);
	}
	printf("\n");
	TCanvas *c4 = new TCanvas("cellavg","cellavg",1200,800);
	c4->cd();
	mg4->GetXaxis()->SetRangeUser(0,1000);
	mg4->GetYaxis()->SetRangeUser(0,4096);
	mg4->Draw("APL");
	c4->SetGridy();
	c4->SetGridx();
	TCanvas *c5 = new TCanvas("celloffsetavg","celloffsetavg",1200,800);
	c5->cd();
	mg5->GetXaxis()->SetRangeUser(0,1000);
	mg5->GetYaxis()->SetRangeUser(-25,25);
	mg5->Draw("APL");
	c5->SetGridy();
	c5->SetGridx();
